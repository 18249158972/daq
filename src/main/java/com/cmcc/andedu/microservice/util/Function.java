...
public class Function {
  ...
  public static String int2String(Integer number) {
        if (number == null) {
            return null;
        }
...
//        System.out.println(getDatetype("2014-11-10 15:23:20"));
	...
	//判断是否是整数
	public static int parseString(String inputNumber) {
    	if (inputNumber == null)
            return -1;
    	else if("".equals(inputNumber)){
    		return -1;
    	}
        try {
        	if (inputNumber.matches("^[0-9]*$")) {
        	    int num=Integer.parseInt(inputNumber.trim());
        		return num;
			}else {
				return -1;
			}
